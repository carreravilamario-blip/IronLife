...
//  analítica del backend (/api/estadisticas). Cae a datos de
//  DEMO si el usuario aún no tiene sesiones registradas.
// ============================================================
...
import {
  PR_LIFTS as LIFTS_DEMO, WEEK_LABELS as WL, WEEKLY_VOLUME as WV, MUSCLE_SPLIT as MS,
  BODYWEIGHT as BW, ACHIEVEMENTS as ACH, OVERVIEW as OV, epley1RM as e1rm,
} from "../data/progresoData";
...
export default function PantallaProgreso() {
  ...
  // Estado de datos: null = cargando, objeto vacío = error/demo.
  ...
  // Decide si hay datos reales o hay que usar demo.
  const tieneRealData = stats && stats.resumen && stats.resumen.sesiones_total > 0;

  // ── Construye LIFTS desde progresion_fuerza de la API ──────
  const LIFTS = tieneRealData && stats.progresion_fuerza && stats.progresion_fuerza.length > 0
  ...
  // Cuando LIFTS cambia (carga de API), resetea la selección al primer elemento.
  ...
  const e1Start = e1rm(start, lift.reps);
  const e1Now = e1rm(now, lift.reps);

  // ── KPIs ────────────────────────────────────────────────────
  const kpiVolumen = tieneRealData
    ? `${(stats.resumen.volumen_total_kg / 1000).toFixed(1)}k`
    : `${(OV.totalVolume / 1000000).toFixed(2)}M`;
  const kpiVolumenUnit = tieneRealData ? "kg" : "kg";
  const kpiSesiones = tieneRealData ? stats.resumen.sesiones_total : OV.sessions;
  const kpiSesionesSub = tieneRealData ? "12 semanas" : `${OV.weeks} semanas`;
  const kpiRacha = tieneRealData ? stats.resumen.racha_actual : OV.streak;
  const kpiHoras = tieneRealData ? Math.round(stats.resumen.horas_total) : OV.hoursTrained;

  // ── Récords ─────────────────────────────────────────────────
  const records = tieneRealData && stats.records && stats.records.length > 0
    ? stats.records.slice(0, 6).map((r, idx) => ({
        ...
        color: API_COLORS[idx % API_COLORS.length],
        reps: r.reps ?? 5,
        // history con un solo punto para que el cálculo de 1RM funcione
        history: [r.kg_max],
      }))
    : LIFTS_DEMO;

  // ── Reparto muscular ─────────────────────────────────────────
  const muscleSplit = tieneRealData && stats.reparto_muscular && stats.reparto_muscular.length > 0
  ...
  // ── Volumen semanal ──────────────────────────────────────────
  const weeklyVolume = tieneRealData && stats.volumen_semanal && stats.volumen_semanal.length > 0
    ? stats.volumen_semanal.map((s) => s.volumen_kg)
    : WV;
  const weekLabels = tieneRealData && stats.volumen_semanal && stats.volumen_semanal.length > 0
    ? stats.volumen_semanal.map((_, i) => `S${i + 1}`)
    : WL;

  // ── Etiquetas para el gráfico de fuerza ─────────────────────
  const forceLabels = tieneRealData && stats.progresion_fuerza && stats.progresion_fuerza.length > 0
  ...
  const bwStart = BW[0], bwNow = BW[BW.length - 1];
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100%", background: G.bg, color: G.text, fontFamily: G.sans }}>
      <header style={{ padding: "24px 32px 22px", borderBottom: `1px solid ${G.line}`, display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16 }}>
        <div>
          <div style={{ fontFamily: G.mono, fontSize: 11, letterSpacing: "0.16em", textTransform: "uppercase", color: G.accent, marginBottom: 8 }}>
            {tieneRealData ? "Análisis · Datos reales" : "Análisis · Demo (sin sesiones aún)"}
          </div>
      ...
      <div style={{ flex: 1, padding: "24px 32px 40px" }}>
        ...
        <div style={{ display: "flex", gap: 13, flexWrap: "wrap", marginBottom: 22 }}>
          <Kpi label="Volumen total" value={kpiVolumen} unit={kpiVolumenUnit} sub="▲ peso movido" accent />
          <Kpi label="Sesiones" value={kpiSesiones} sub={kpiSesionesSub} />
          <Kpi label="Racha actual" value={kpiRacha} unit="días" />
          <Kpi label="Horas" value={kpiHoras} unit="h" />
          {!tieneRealData && <Kpi label="Cardio" value={OV.cardioKm} unit="km" />}
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "minmax(0,1.7fr) minmax(300px,1fr)", gap: 20, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <Card title="Progresión de fuerza" pad={20}
              ...
              <LineChart data={lift.history} labels={forceLabels.length === lift.history.length ? forceLabels : WL} unit="kg" color={lift.color} height={250} />
            </Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* peso corporal */}
            <Card title="Peso corporal" action={<span style={{ fontFamily: G.mono, fontSize: 12, fontWeight: 700, color: G.green }}>{bwStart} → {bwNow} kg</span>}>
              <LineChart data={BW} labels={WL} unit="kg" color="#38bdf8" height={150} annotate={false} />
            </Card>
          </div>
        </div>
